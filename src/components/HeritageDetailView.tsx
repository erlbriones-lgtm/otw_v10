...
export default function HeritageDetailView({ heritage, onBack }: HeritageDetailViewProps) {
  ...
  return (
    <div className="w-full max-w-7xl mx-auto px-6 sm:px-12 pt-32 pb-24 text-left text-[#05461a]" id="heritage-detail-view-container">
      ...
      <button
        onClick={onBack}
        className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-transparent border border-[#05461a]/20 text-[#05461a] hover:bg-[#05461a]/5 transition-all font-mono text-xs uppercase tracking-wider mb-8 cursor-pointer active:scale-95 btn-secondary-custom"
      >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start mb-12">
        <div className="lg:col-span-12">
          <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-[#05461a]/10 text-[#05461a] border border-[#05461a]/25 rounded-full font-mono text-[10px] uppercase tracking-widest font-extrabold mb-4 badge-tag-custom">
            <Compass className="w-3.5 h-3.5 text-[#05461a] animate-spin-slow" /> {heritage.category}
          </span>
          <h1 className="font-display font-black text-[#05461a] text-3xl sm:text-5xl md:text-6xl tracking-tight leading-none mb-4 animate-fade-in">
            {heritage.title}
          </h1>
          <p className="text-[#2E7D32] text-base sm:text-lg font-sans max-w-2xl leading-relaxed font-semibold">
            "{heritage.tagline}"
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
        ...
        <div className="lg:col-span-6 flex flex-col gap-5">
          <div className="relative h-[320px] sm:h-[420px] rounded-3xl overflow-hidden border border-emerald-100 bg-emerald-50/20 flex items-center justify-center">
            ...
            <div className="absolute bottom-4 left-4 flex items-center gap-2 bg-black/45 backdrop-blur-md px-3 py-1.5 rounded-xl border border-white/10">
              <MapPin className="w-3.5 h-3.5 text-[#FFD54F] fill-[#FFD54F]/15" />
          ...
          <div>
            <span className="text-[10px] font-mono text-gray-500 uppercase tracking-widest font-bold block mb-2.5">
        ...
        <div className="lg:col-span-6 flex flex-col gap-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="p-5 rounded-2xl bg-[#05461a]/5 border border-emerald-100 text-left shadow-sm backdrop-blur-sm">
              ...
              <span className="font-mono text-[10px] text-gray-500 uppercase tracking-widest font-bold block">YEAR ANCHORED</span>
            ...
            <div className="p-5 rounded-2xl bg-[#05461a]/5 border border-emerald-100 text-left shadow-sm backdrop-blur-sm">
              ...
              <span className="font-mono text-[10px] text-gray-500 uppercase tracking-widest font-bold block">DISTRICT REGION</span>
              <span className="text-[#05461a] text-base sm:text-lg font-sans font-black mt-1 block">
                {heritage.district}
              </span>
            </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {heritage.facts.map((fact, idx) => (
                      <div key={idx} className="p-3.5 rounded-xl bg-emerald-50/40 border border-emerald-100 text-left">
                        <span className="text-[10px] font-mono text-gray-400 block font-bold uppercase">{fact.label}</span>
                        <span className="text-[#05461a] text-xs sm:text-sm font-sans font-semibold mt-0.5 block">{fact.value}</span>
                      </div>
                    ))}
                  </div>

                  <div className="space-y-2 mt-4">
                    <span className="text-[10px] font-mono text-gray-400 uppercase block font-bold">HERITAGE VALOR HIGHLIGHTS</span>
                    <ul className="space-y-1.5 pl-1 text-xs text-gray-650 font-medium">
              ...
              {activeTab === "visit" && (
                <div className="space-y-4 animate-fade-in">
                  <div>
                    ...
                    <p className="text-gray-400 text-[11px] mt-0.5 font-semibold">Please observe proper manners when checking physical spots</p>
                  </div>

                  <div className="bg-emerald-50/40 border border-emerald-100 p-4 rounded-xl text-xs text-gray-600 space-y-2 font-medium">
                    {heritage.culturalGuidelines.map((g, idx) => (
                      <div key={idx} className="flex gap-2.5 items-start">
                        <span className="text-[#05461a] font-mono text-xs">0{idx + 1}.</span>
                        <span>{g}</span>
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="border-t border-gray-100 pt-4">
                    <span className="text-[10px] font-mono text-gray-400 uppercase block font-bold">TRAVELER PLANNING INSIGHTS</span>
                    <p className="text-xs sm:text-sm text-gray-600 leading-relaxed mt-1.5 font-medium">
                      {heritage.travelTips}
                    </p>
                  </div>
                </div>
